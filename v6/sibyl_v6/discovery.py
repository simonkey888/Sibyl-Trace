...
def _tokens(title: str) -> set[str]:
    stop = {"will", "the", "be", "a", "an", "at", "on", "by", "to", "of", "in", "is", "yes", "no"}
    return {x for x in re.findall(r"[a-z0-9.$%]+", title.casefold()) if len(x) > 1 and x not in stop}
...
def discover_candidates(limit_per_venue: int = 100, minimum_similarity: float = 0.35) -> list[CandidatePair]:
    _, lmts_payload = _get_json("https://api.limitless.exchange/markets/active?limit=25&page=1")
    ...
    for lrow in lmts[:limit_per_venue]:
        ...
        for prow in poly[:limit_per_venue]:
            ...
            if ptitle and pid and score >= minimum_similarity:
                out.append(CandidatePair(lslug, pid, ltitle, ptitle, round(score, 6)))
    return sorted(out, key=lambda row: (-row.title_similarity, row.limitless_slug, row.polymarket_id))
...
def load_verified_pairs(path: Path) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        left = MarketDescriptor("LIMITLESS", str(row["limitless_slug"]), str(row["limitless_title"]), left_rule)
        right = MarketDescriptor("POLYMARKET", str(row["polymarket_id"]), str(row["polymarket_title"]), right_rule)
        ...
        if comparison.state is not PairState.EXACT_EQUIVALENT:
            raise RuntimeError(
                ...
                f"{comparison.state.value} {comparison.differing_fields or comparison.unknown_fields}"
            )
...
def build_discovery_evidence(output: Path, verified_pairs_path: Path) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "SIBYL_V6_PAIR_DISCOVERY_V1",
