...
def _unavailable_profile(wallet: str, *, cutoff_at: int, policy: SourceStrategyPolicy, error_type: str) -> dict:
    material = {
        "wallet_hash": wallet_hash(wallet), "classification": UNAVAILABLE,
        "rejection_reason": "source_strategy_unavailable", "cutoff_at": cutoff_at,
        "event_count": 0, "invalid_timestamp_event_count": 0, "trade_count": 0,
        "attributable_trade_count": 0, "unattributable_trade_count": 0,
        "maker_rebate_count": 0, "taker_rebate_count": 0, "split_count": 0,
        "merge_count": 0, "conversion_count": 0, "paired_condition_count": 0,
        "paired_trade_count": 0, "paired_trade_fraction": 0.0,
...
def scan_wallets(db: Session, client: PolymarketClient, settings: Settings, *, prospective: bool = False, source_strategy_gate: bool = False) -> list[Wallet]:
    ...
    for address, candidate in candidates.items():
        ...
        matrix = score_matrix(db, address, closed, volume=float(candidate["vol"]))
        ...
        db.add(profile)
        audit(db, "wallet_quality_scored", "Persist reconstructable heuristic source-quality evidence",
              wallet=address, score_kind=QUALITY_SCORE_KIND, score_global_formula=QUALITY_SCORE_GLOBAL_FORMULA,
              score_history_basis=QUALITY_SCORE_HISTORY_BASIS, calibrated_probability=QUALITY_SCORE_CALIBRATED_PROBABILITY,
              expected_return_claim=QUALITY_SCORE_EXPECTED_RETURN_CLAIM, alpha_claim=QUALITY_SCORE_ALPHA_CLAIM,
              short_closed_count=matrix.short_metrics.closed_count, short_decided_count=matrix.short_metrics.decided_count,
              short_score=matrix.short_score, long_closed_count=matrix.long_metrics.closed_count,
              long_decided_count=matrix.long_metrics.decided_count, long_score=matrix.long_score,
              global_score=matrix.global_score, win_rate_denominator="wins_plus_losses", rejection_reason=matrix.rejection_reason)
        db.add(WalletSnapshot(wallet_address=address, score=matrix.global_score, win_rate=metrics.win_rate,
                              profit_factor=metrics.profit_factor, realized_pnl=metrics.realized_pnl,
                              concentration=metrics.concentration, closed_count=metrics.closed_count))
    ...
    if source_strategy_gate:
        ...
        policy = SourceStrategyPolicy(min_trade_count=settings.source_strategy_min_trade_count,
        ...
        for wallet in ranked:
            ...
            try:
                ...
                strategy_payload = classify_source_strategy(
                    wallet.address, events, cutoff_at=cutoff_at, policy=policy
                ).to_dict()
                forensics_payload = compute_wallet_forensics(
                    ...
                    scan_realized_pnl_percentile=_reported_pnl_percentile(wallets, wallet),
                ).to_dict()
            except Exception as exc:
            ...
            forensic_row = db.get(WalletForensicsProfile, wallet.address) or WalletForensicsProfile(
            ...
            audit(
                ...
                maker_taker_ratio_proven=forensics_payload.get("maker_taker_ratio_proven", False),
    ...
    audit(db, "wallet_scan_completed", f"Scored {len(wallets)} wallets; selected {len(eligible)}",
          candidates=len(wallets), selected=[wallet.address for wallet in eligible],
          score_contract="HEURISTIC GLOBAL=60% SHORT + 40% LONG; HISTORY=DECIDED_OUTCOMES; EDGE=execution copyability only",
          score_calibrated_probability=False, score_expected_return_claim=False, score_alpha_claim=False,
          selection_mode="PROSPECTIVE_ONLY" if prospective else "LEGACY_SCAN_THEN_INGEST",
          selection_effective_at=selection_effective_at, source_strategy_gate=source_strategy_gate,
