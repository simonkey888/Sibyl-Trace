...
from app.source_strategy import UNAVAILABLE, SourceStrategyPolicy, canonical_hash, classify_source_strategy, fetch_public_activity_events, wallet_hash
from app.wallet_forensics import FORENSICS_SCHEMA_VERSION, compute_wallet_forensics, fetch_public_execution_mix, unavailable_wallet_forensics
...
def _unavailable_profile(wallet: str, *, cutoff_at: int, policy: SourceStrategyPolicy, error_type: str) -> dict:
    material = {"wallet_hash": wallet_hash(wallet), "classification": UNAVAILABLE, "rejection_reason": "source_strategy_unavailable", "cutoff_at": cutoff_at, "event_count": 0, "invalid_timestamp_event_count": 0, "trade_count": 0, "attributable_trade_count": 0, "unattributable_trade_count": 0, "maker_rebate_count": 0, "taker_rebate_count": 0, "split_count": 0, "merge_count": 0, "conversion_count": 0, "paired_condition_count": 0, "paired_trade_count": 0, "paired_trade_fraction": 0.0, "activity_sample_hash": canonical_hash([]), "policy": {"min_trade_count": policy.min_trade_count, "min_paired_conditions": policy.min_paired_conditions, "max_paired_trade_fraction": policy.max_paired_trade_fraction}, "error_type": error_type}
    return {**material, "directional": False, "evidence_hash": canonical_hash(material)}


def scan_wallets(db: Session, client: PolymarketClient, settings: Settings, *, prospective: bool = False, source_strategy_gate: bool = False) -> list[Wallet]:
    ...
    dict_rows_by_wallet: dict[str, list[dict]] = {}
    ...
    for address, candidate in candidates.items():
        ...
        dict_rows_by_wallet[address] = [item for item in evidence.rows if isinstance(item, dict)]
        matrix = score_matrix(db, address, evidence.rows, volume=float(candidate["vol"]), history_evidence=evidence.history, code_sha=code_sha)
        ...
        audit(db, "wallet_quality_scored", "Persist reconstructable heuristic source-quality evidence", wallet=address, score_kind=QUALITY_SCORE_KIND, score_global_formula=QUALITY_SCORE_GLOBAL_FORMULA, score_history_basis=QUALITY_SCORE_HISTORY_BASIS, calibrated_probability=QUALITY_SCORE_CALIBRATED_PROBABILITY, expected_return_claim=QUALITY_SCORE_EXPECTED_RETURN_CLAIM, alpha_claim=QUALITY_SCORE_ALPHA_CLAIM, short_closed_count=matrix.short_metrics.closed_count, short_decided_count=matrix.short_metrics.decided_count, short_score=matrix.short_score, long_closed_count=matrix.long_metrics.closed_count, long_decided_count=matrix.long_metrics.decided_count, long_score=matrix.long_score, global_score=matrix.global_score, win_rate_denominator="wins_plus_losses", rejection_reason=matrix.rejection_reason, history_status=evidence.history.status, history_scope=evidence.history.scope, history_has_more=evidence.history.has_more, history_source_hash=evidence.history.source_hash, score_input_hash=matrix.provenance["input_hash"], score_algorithm_version=matrix.provenance["algorithm_version"], score_code_sha=code_sha)
    ...
    if source_strategy_gate:
        ...
        policy = SourceStrategyPolicy(min_trade_count=settings.source_strategy_min_trade_count, min_paired_conditions=settings.source_strategy_min_paired_conditions, max_paired_trade_fraction=settings.source_strategy_max_paired_trade_fraction)
        ...
        for wallet in ranked:
            ...
            try:
                ...
                strategy_payload = classify_source_strategy(wallet.address, events, cutoff_at=cutoff_at, policy=policy).to_dict()
