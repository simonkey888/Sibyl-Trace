...
main{{max-width:1180px;margin:auto;padding:32px}}h1{{margin:0 0 6px}}p{{color:#a9b6c4}}
...
.card b{{display:block;font-size:1.5rem;margin-top:8px}}table{{width:100%;border-collapse:collapse}}
th,td{{text-align:left;padding:9px;border-bottom:1px solid #253443}}code{{color:#8bd5ff}}
small{{color:#8ea0b2}}.warning{{border-left:4px solid #d6a84b}}
</style>
</head>
<body><main>
...
<tr><th>Executable divergences</th><td>{escape(_value(latency.get('executable_divergences', 0)))}</td></tr>
<tr><th>Average lag ms</th><td>{escape(_value(latency.get('average_lag_ms')))}</td></tr>
<tr><th>Average executable edge/share</th><td>{escape(_value(latency.get('average_executable_edge_per_share')))}</td></tr>
</tbody></table></section>
<section><h2>Reference trader reconstructions</h2><table>
<thead><tr><th>Trader</th><th>Status</th><th>n</th><th>Win rate</th><th>Payoff ratio</th><th>Expectancy R</th></tr></thead>
<tbody>{''.join(trader_rows)}</tbody></table></section>
<section><h2>Preregistered hypotheses</h2><ul>{hypothesis_items}</ul></section>
<section class="warning"><strong>Safety invariant</strong><p>No private key, signing, deposit, withdrawal, live order, paid API or billing action is available from this artifact.</p></section>
</main></body></html>"""
