...
def compute_wallet_forensics(
    ...
    *,
    ...
    if (
        mix_proven
        and int(mix.get("sample_total") or 0) >= EXECUTION_STYLE_MIN_SAMPLE
        and maker_ratio is not None
    ):
        if float(maker_ratio) >= 0.80:
            execution_style = "MAKER_HEAVY"
        elif float(maker_ratio) <= 0.20:
            execution_style = "TAKER_HEAVY"
        else:
    ...
    material: dict[str, Any] = {
        ...
        "selection_veto_reason": None,
        "execution_style": execution_style,
        "execution_style_min_sample": EXECUTION_STYLE_MIN_SAMPLE,
